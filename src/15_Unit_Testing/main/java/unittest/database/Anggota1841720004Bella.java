...
public class Anggota1841720004Bella {
    ...
    ArrayList<Anggota1841720004Bella> getByNamaAndAlamatBella(String nama, String alamat) {
        ArrayList<Anggota1841720004Bella> ListAnggota = new ArrayList();
        ResultSet rs;
        if ((nama.trim().length()>0) && (alamat.trim().length()>0)){
            rs = DBHelper1841720004Bella.selectQuery("SELECT * FROM anggota Where nama = '"+
            nama +"' and alamat = '"+ alamat+"'");
        }else if ((nama.trim().length()>0) && (alamat.trim().length()==0)){
            rs = DBHelper1841720004Bella.selectQuery("SELECT * FROM anggota Where nama = '"+
            nama+"'");
        }else {
            rs = DBHelper1841720004Bella.selectQuery("SELECT * FROM anggota Where alamat = '"+ alamat+"'");
        }
        try {
            while (rs.next()) {
                Anggota1841720004Bella ang = new Anggota1841720004Bella();
